...
function solution() {
  ...
  for(let i=0; i<goalChannel.length; i++) {
    if(errorBtn[goalChannel[i]] !== -1) { // 고장난 버튼 x
      if(s_flag) {
      ...
      if(!s_flag && !b_flag) { // 아직 한번도 다른적이 없다
        for(let k=1; ;k++) {
          // 고장난 버튼 아니면 
      ...
      } else if(s_flag) { // 앞에서 작은 수를 저장한 경우 
        // 넣을 수 있는 값 중 최대값 넣기
        curChannel[i] = maxValue;
      } else if(b_flag) { // 앞에서 큰 수를 저장한 경우
        curChannel[i] = minValue;
      }
    }
    console.log(curChannel);
  }
  answer = Math.abs(Number(goalChannel.join(''))-Number(curChannel.join('')))+goalChannel.length;
  return answer;
}
